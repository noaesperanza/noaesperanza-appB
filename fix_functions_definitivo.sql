-- 🔧 CORREÇÃO DEFINITIVA: Remover todas as versões das funções

-- PASSO 1: Listar e remover TODAS as versões de register_noa_conversation
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Buscar todas as versões da função register_noa_conversation
    FOR func_record IN 
    ...
    LOOP
        ...
        EXECUTE format('DROP FUNCTION IF EXISTS %s(%s) CASCADE', 
                      func_record.proname, 
                      array_to_string(func_record.proargtypes, ', '));
        RAISE NOTICE 'Removida função: % com % argumentos', func_record.proname, func_record.pronargs;
    END LOOP;
END $$;

-- PASSO 2: Listar e remover TODAS as versões de save_ai_learning
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Buscar todas as versões da função save_ai_learning
    FOR func_record IN 
        SELECT proname, pronargs, proargtypes::regtype[]
        FROM pg_proc 
...
SELECT 'Funções register_noa_conversation e save_ai_learning criadas com sucesso!' as resultado;
