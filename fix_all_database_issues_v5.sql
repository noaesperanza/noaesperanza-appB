-- =====================================================
-- SCRIPT CORRIGIDO V5: Fix All Database Issues
-- =====================================================
-- Este script corrige todos os problemas identificados:
-- 1. Adiciona colunas ausentes
-- 2. Remove índices GIN problemáticos antes da conversão
-- 3. Converte tipos de dados (text[] → text) de forma segura
-- 4. Remove duplicatas
-- 5. Corrige UUIDs inválidos
-- 6. Cria constraints necessárias
-- 7. Recria índices apropriados (BTREE para text)
-- =====================================================
...
-- 2. REMOVER ÍNDICES GIN PROBLEMÁTICOS ANTES DA CONVERSÃO
-- =====================================================

-- Verificar e remover índices GIN existentes nas colunas keywords
DO $$ 
DECLARE
    idx_record RECORD;
BEGIN
    -- Buscar índices GIN relacionados a keywords nas nossas tabelas
    FOR idx_record IN 
...
-- 8. RECRIAR ÍNDICES APROPRIADOS PARA TEXT
-- =====================================================

-- Recriar índice BTREE para ai_learning.keywords (agora como TEXT)
DO $$ 
BEGIN 
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Criar índice BTREE para busca de texto (mais apropriado para text)
        CREATE INDEX IF NOT EXISTS ai_learning_keywords_btree_idx 
        ON ai_learning USING btree (keywords);
        RAISE NOTICE 'Índice BTREE criado para ai_learning.keywords';
    END IF;
END $$;

-- Recriar índice BTREE para ai_conversation_patterns.keywords (agora como TEXT)
DO $$ 
BEGIN 
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Criar índice BTREE para busca de texto (mais apropriado para text)
        CREATE INDEX IF NOT EXISTS ai_conversation_patterns_keywords_btree_idx 
        ON ai_conversation_patterns USING btree (keywords);
        RAISE NOTICE 'Índice BTREE criado para ai_conversation_patterns.keywords';
    END IF;
END $$;
...
-- ✅ Índices GIN problemáticos removidos antes da conversão
...
-- ✅ Índices BTREE apropriados para text criados
