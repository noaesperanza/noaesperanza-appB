...
export interface UserIntent {
  type: 'start_evaluation' | 'ask_about_method' | 'greeting' | 'presentation' | 'symptom' | 'question' | 'unknown'
  confidence: number
  detectedProfile?: 'paciente' | 'profissional' | 'aluno'
  context?: any
}
...
export class UserIntentDetector {
  ...
  static detectUserPresentation(message: string, allMessages: any[]): boolean {
    ...
    const hasName = lower.includes('meu nome') || 
                    lower.includes('me chamo') || 
                    lower.includes('sou ') ||
                    lower.match(/\b[A-Z][a-z]+ [A-Z][a-z]+\b/) // Nome próprio
    
    // Verifica histórico de mensagens
    const hasIntroducedBefore = allMessages.some(msg => 
      msg.sender === 'user' && 
  ...
  static extractContext(allMessages: any[]): {
    ...
    // Extrai nome
    const nameMessages = allMessages.filter(msg => 
      msg.sender === 'user' && 
      (msg.message.toLowerCase().includes('meu nome') || msg.message.toLowerCase().includes('me chamo'))
    )
    if (nameMessages.length > 0) {
      const match = nameMessages[0].message.match(/(?:meu nome é|me chamo) ([A-Z][a-z]+(?: [A-Z][a-z]+)*)/i)
      if (match) context.userName = match[1]
    }
    
    // Extrai sintomas
    const symptomMessages = allMessages.filter(msg =>
      msg.sender === 'user' &&
