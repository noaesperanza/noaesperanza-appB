...
export class SemanticAttentionService {
  ...
  async processUserInput(input: string, userContext: UserContext): Promise<ProcessedInput> {
    try {
      ...
      const attentionScores = this.calculateAttentionScores(
        semanticFeatures,
        userContext.attentionWeights
      )
  ...
  private async createUserContext(userId: string, userName: string): Promise<UserContext> {
    ...
    // Salvar no banco
    await supabase.from('user_semantic_context').insert({
      user_id: userId,
      user_name: userName,
  ...
  private async updateAttentionWeights(userContext: UserContext): Promise<void> {
    ...
    if (conversations && conversations.length > 0) {
      ...
      const technicalCount = conversations.filter(
        c => c.content.includes('método') || c.content.includes('análise')
      ).length
      const medicalCount = conversations.filter(
        c => c.content.includes('paciente') || c.content.includes('tratamento')
      ).length
      const researchCount = conversations.filter(
        c => c.content.includes('pesquisa') || c.content.includes('estudo')
      ).length
  ...
  private calculateAttentionScores(
  ...
  // 🎯 GERAR CONTEXTO FOCADO
  private generateFocusedContext(
    input: string,
  ...
  private countTechnicalTerms(input: string): number {
    ...
    return (
      technicalTerms.filter(term => input.toLowerCase().includes(term)).length /
  ...
  private detectMedicalContext(input: string): number {
    ...
    return (
      medicalTerms.filter(term => input.toLowerCase().includes(term)).length / medicalTerms.length
    )
  }
  ...
  private detectResearchFocus(input: string): number {
    ...
    return (
      researchTerms.filter(term => input.toLowerCase().includes(term)).length / researchTerms.length
    )
  }
  ...
  private detectPersonalContext(input: string): number {
    ...
    return (
      personalTerms.filter(term => input.toLowerCase().includes(term)).length / personalTerms.length
    )
  }
  ...
  private calculateTemporalRelevance(input: string): number {
    ...
    return (
      temporalTerms.filter(term => input.toLowerCase().includes(term)).length / temporalTerms.length
    )
  }
  ...
  private getDominantContext(scores: AttentionScores): string {
    ...
    const dominantKey = Object.keys(scores).find(
      key => scores[key as keyof AttentionScores] === maxScore
    )
  ...
  private determineResponseStrategy(dominantContext: string, userContext: UserContext): string {
    const strategies = {
      technical:
        userContext.semanticProfile.preferredDepth === 'comprehensive'
          ? 'análise detalhada'
          : 'explicação concisa',
  ...
  private async generateResponse(prompt: string): Promise<string> {
    ...
    try {
      // Usar codexService para gerar resposta real
      const response = await codexService.getNoaResponse(prompt, [], {
        route: 'chat',
        metadata: { origin: 'semanticAttention' },
      })
      return response
    } catch (error) {
...
Como posso ajudá-lo especificamente hoje?`
  ...
  private async updateSemanticProfile(
...
export async function processUserMessage(message: string, userContext: string) {
  ...
  const response = await codexService.getNoaResponse(symbolicPrompt, [], {
    route: 'chat',
    metadata: { origin: 'semanticAttention.symbolic' },
  })
  return response
}
