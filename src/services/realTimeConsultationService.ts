  ...
  async consultKnowledgeBase(context: ConsultationContext): Promise<ConsultationResult> {
    try {
      ...
      return {
        query: context.userQuery,
        documents: [],
        answer: 'Desculpe, não consegui consultar a base de conhecimento no momento. Por favor, tente novamente.',
        sources: [],
        timestamp: new Date(),
        confidence: 0
      }
  ...
  private async findRelevantDocuments(query: string): Promise<DocumentMaster[]> {
    try {
      ...
      for (const keyword of keywords) {
        ...
        keywordResults.push(...results.filter(doc => 
      ...
      for (const category of medicalCategories) {
        ...
        categoryResults.push(...results.filter(doc => 
          !directResults.some(existing => existing.id === doc.id) &&
...
Responda de forma clara e útil:
  ...
  private extractKeywords(query: string): string[] {
    const medicalKeywords = [
      'neurologia', 'cannabis', 'nefrologia', 'avaliação', 'clínica',
      'entrevista', 'método', 'valença', 'nôa', 'esperanza',
      'documento', 'mestre', 'base', 'conhecimento', 'instruções'
    ]
  ...
  private calculateRelevanceScore(document: DocumentMaster, query: string): number {
    ...
    const contentMatches = (document.content.toLowerCase().match(new RegExp(query, 'g')) || []).length
...
O que você gostaria de fazer?`
  ...
  getConsultationStats(): {
    ...
    const averageConfidence = this.consultationHistory.reduce(
      (sum, consultation) => sum + consultation.confidence, 0
    ) / totalConsultations
