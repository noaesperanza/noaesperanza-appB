-- =====================================================
-- SCRIPT CORRIGIDO: Fix All Database Issues V2
-- =====================================================
-- Este script corrige todos os problemas identificados:
-- 1. Adiciona colunas ausentes
-- 2. Corrige tipos de dados
-- 3. Remove duplicatas
-- 4. Corrige UUIDs inválidos (usando UUIDs existentes)
-- 5. Cria constraints necessárias
-- =====================================================
...
-- 2. VERIFICAR E CORRIGIR TIPOS DE DADOS
-- =====================================================

-- Verificar se a coluna keywords em ai_learning é text[] e corrigir para text
DO $$ 
BEGIN 
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Remover índice GIN se existir
        IF EXISTS (
            SELECT 1 FROM pg_indexes 
            WHERE tablename = 'ai_learning' 
            AND indexname LIKE '%keywords%'
        ) THEN
            DROP INDEX IF EXISTS ai_learning_keywords_gin_idx;
            RAISE NOTICE 'Índice GIN removido da coluna keywords em ai_learning';
        END IF;
...
-- Verificar se a coluna keywords em ai_conversation_patterns é text[] e corrigir para text
DO $$ 
BEGIN 
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        IF EXISTS (
            SELECT 1 FROM pg_indexes 
            WHERE tablename = 'ai_conversation_patterns' 
            AND indexname LIKE '%keywords%'
        ) THEN
            DROP INDEX IF EXISTS ai_conversation_patterns_keywords_gin_idx;
            RAISE NOTICE 'Índice GIN removido da coluna keywords em ai_conversation_patterns';
        END IF;
...
-- 3. CORRIGIR UUIDs INVÁLIDOS (USANDO UUIDs EXISTENTES)
-- =====================================================
...
-- 4. REMOVER DUPLICATAS EM ai_learning
-- =====================================================
...
-- 5. REMOVER DUPLICATAS EM ai_keywords
-- =====================================================
...
-- 6. RECRIAR ÍNDICES GIN APROPRIADOS
-- =====================================================
...
-- 9. VERIFICAR TABELAS PRINCIPAIS
-- =====================================================
...
-- 10. VERIFICAR COLUNAS PRINCIPAIS
-- =====================================================
...
-- ✅ Índices GIN removidos e recriados apropriadamente
