-- ========================================
-- SQL COMPLETO PARA SUPABASE - Nôa Esperanza 2.0
...
DROP POLICY IF EXISTS "Leitura pública documentos" ON documentos_mestres;
CREATE POLICY "Leitura pública documentos" ON documentos_mestres FOR SELECT USING (is_active = true);

DROP POLICY IF EXISTS "Leitura pública config" ON noa_config;
CREATE POLICY "Leitura pública config" ON noa_config FOR SELECT USING (true);

DROP POLICY IF EXISTS "Leitura pública interacoes" ON interacoes_noa;
CREATE POLICY "Leitura pública interacoes" ON interacoes_noa FOR SELECT USING (true);

DROP POLICY IF EXISTS "Leitura pública mensagens" ON mensagens_noa;
CREATE POLICY "Leitura pública mensagens" ON mensagens_noa FOR SELECT USING (true);

DROP POLICY IF EXISTS "Leitura pública auditoria" ON auditoria_simbolica;
CREATE POLICY "Leitura pública auditoria" ON auditoria_simbolica FOR SELECT USING (true);

DROP POLICY IF EXISTS "Leitura pública estatisticas" ON estatisticas_migracao;
CREATE POLICY "Leitura pública estatisticas" ON estatisticas_migracao FOR SELECT USING (true);
