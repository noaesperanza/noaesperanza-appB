-- 🔧 CORREÇÃO: Funções ausentes no Supabase

-- 1. CRIAR/SUBSTITUIR FUNÇÃO register_noa_conversation
-- Remover todas as versões possíveis da função
DROP FUNCTION IF EXISTS register_noa_conversation CASCADE;
CREATE OR REPLACE FUNCTION register_noa_conversation(
  session_id_param TEXT,
  user_id_param TEXT,
  user_message_param TEXT,
  ai_response_param TEXT,
  user_type_param TEXT DEFAULT 'paciente'
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Inserir conversa na tabela noa_conversations
  INSERT INTO noa_conversations (
    session_id,
    user_id,
    user_message,
    ai_response,
    user_type,
    created_at
  ) VALUES (
    session_id_param,
    user_id_param,
    user_message_param,
    ai_response_param,
    user_type_param,
    NOW()
  );
  
  -- Retornar sucesso
  RETURN json_build_object(
    'success', true,
    'message', 'Conversa registrada com sucesso'
  );
  
EXCEPTION
  WHEN OTHERS THEN
    RETURN json_build_object(
      'success', false,
      'error', SQLERRM
    );
END;
$$;

-- 2. CORRIGIR FUNÇÃO save_ai_learning (usar ON CONFLICT)
-- Remover todas as versões possíveis da função
DROP FUNCTION IF EXISTS save_ai_learning CASCADE;
CREATE OR REPLACE FUNCTION save_ai_learning(
  keyword_param TEXT,
  context_param TEXT,
  user_message_param TEXT,
  ai_response_param TEXT,
  category_param TEXT,
  confidence_score_param NUMERIC,
  user_id_param TEXT
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Inserir ou atualizar aprendizado com ON CONFLICT
  INSERT INTO ai_learning (
    keyword,
    context,
    user_message,
    ai_response,
    category,
    confidence_score,
    user_id,
    created_at,
    updated_at
  ) VALUES (
    keyword_param,
    context_param,
    user_message_param,
    ai_response_param,
    category_param,
    confidence_score_param,
    user_id_param,
    NOW(),
    NOW()
  )
  ON CONFLICT (keyword, category) 
  DO UPDATE SET
    context = EXCLUDED.context,
    user_message = EXCLUDED.user_message,
    ai_response = EXCLUDED.ai_response,
    confidence_score = EXCLUDED.confidence_score,
    user_id = EXCLUDED.user_id,
    updated_at = NOW(),
    usage_count = ai_learning.usage_count + 1;
  
  -- Retornar sucesso
  RETURN json_build_object(
    'success', true,
    'message', 'Aprendizado salvo/atualizado com sucesso'
  );
  
EXCEPTION
  WHEN OTHERS THEN
    RETURN json_build_object(
      'success', false,
      'error', SQLERRM
    );
END;
$$;

-- 3. VERIFICAR SE TABELA noa_conversations EXISTE
CREATE TABLE IF NOT EXISTS noa_conversations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id TEXT NOT NULL,
  user_id TEXT NOT NULL,
  user_message TEXT NOT NULL,
  ai_response TEXT NOT NULL,
  user_type TEXT DEFAULT 'paciente',
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 4. CRIAR ÍNDICES PARA PERFORMANCE
CREATE INDEX IF NOT EXISTS idx_noa_conversations_user_id 
ON noa_conversations(user_id);

CREATE INDEX IF NOT EXISTS idx_noa_conversations_session_id 
ON noa_conversations(session_id);

CREATE INDEX IF NOT EXISTS idx_noa_conversations_created_at 
ON noa_conversations(created_at);

-- 5. HABILITAR RLS PARA noa_conversations
ALTER TABLE noa_conversations ENABLE ROW LEVEL SECURITY;

-- 6. CRIAR POLÍTICA RLS
DROP POLICY IF EXISTS "Users can view their own conversations" ON noa_conversations;
CREATE POLICY "Users can view their own conversations" 
ON noa_conversations FOR SELECT 
USING (auth.uid()::text = user_id::text);

DROP POLICY IF EXISTS "Users can insert their own conversations" ON noa_conversations;
CREATE POLICY "Users can insert their own conversations" 
ON noa_conversations FOR INSERT 
WITH CHECK (auth.uid()::text = user_id::text);

-- 7. VERIFICAR E CORRIGIR ai_learning
CREATE INDEX IF NOT EXISTS idx_ai_learning_keyword_category 
ON ai_learning(keyword, category);

-- 8. ATUALIZAR TRIGGER updated_at para noa_conversations
DROP TRIGGER IF EXISTS update_noa_conversations_updated_at ON noa_conversations;
CREATE TRIGGER update_noa_conversations_updated_at
  BEFORE UPDATE ON noa_conversations
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- 9. COMENTÁRIOS FINAIS
COMMENT ON FUNCTION register_noa_conversation IS 'Registra conversas da Nôa no banco de dados';
COMMENT ON FUNCTION save_ai_learning IS 'Salva aprendizados da IA com tratamento de conflitos';
COMMENT ON TABLE noa_conversations IS 'Armazena todas as conversas com a Nôa Esperanza';
